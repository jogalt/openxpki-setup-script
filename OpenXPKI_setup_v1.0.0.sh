...
if [ ! -d "/etc/openxpki/config.d/realm/${REALM}" ]; then
    ...
    cp -R /etc/openxpki/config.d/realm/democa/* /etc/openxpki/config.d/realm/${REALM}
fi
...
ROOT_CA_CERTIFICATE_URI="URI"':''http://'"${FQDN}"'/'"${ROOT_CA}"'.cer'
ROOT_CA_REVOCATION_URI="URI"':''http://'"${FQDN}"'/'"${ROOT_CA}"'.crl'
ISSUING_CERTIFICATE_URI="URI"':''http://'"${FQDN}"'/'"${ISSUING_CA}"'.cer'
ISSUING_REVOCATION_URI="URI"':''http://'"${FQDN}"'/'"${ISSUING_CA}"'.crl'
...
select opt in First_Run Generate_new_Root_CA Generate_new_Intermediate_CA Generate_new_Datavault_Certificate Generate_new_Scep_Certificate Generate_new_Web_Certificate Quit; do
case $opt in
First_Run)							## First_run
 import_xpki_Root="1"
 import_xpki_Inter="1"
...
echo "Remove previous vault-1.pem"
v_KEY_FILE=${vault_dir}vault-1.pem
if [ -f "$v_KEY_FILE" ];
then
 echo "Backing up previous vault key, if found."
 timeStamp="$(echo -e `date` | tr -d '[:space:]' | tr -d '[:]' )" 
 mv ${vault_dir}vault-1.pem ${vault_dir}vault-1.pem."${REALM}"."${timeStamp}".bak
fi
mv ${DATAVAULT_PEM} ${vault_dir}vault-1.pem
...
" >> /etc/openxpki/config.d/realm/${REALM}/crypto.yaml
 ...
 echo -e "from the democa realm directory, we're going to prep it for operation. "
